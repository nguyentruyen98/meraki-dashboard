...
class BaseAP extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      clients: [],
  ...
  componentDidMount() {
    ApiCall('https://api.meraki.com/api/v1/networks/L_566327653141843049/clients?timespan=3600&perPage=300', 'GET', null).then(res => {
      ...
      return (
        ApiCall('https://api.meraki.com/api/v1/devices/Q2LD-3Y7V-7Y2X/clients?timespan=2592000', 'GET', null).then(res => {
          this.setState({
  ...
  showCurrentClient(clients) {
    ...
    if (clients.length > 0) {
      result = clients.map((client, index) => {
        if (client.status == "Online" && client.recentDeviceSerial == "Q2LD-3Y7V-7Y2X") {
